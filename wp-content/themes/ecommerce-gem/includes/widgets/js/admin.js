...
jQuery(function($){
  ////////////////////////////////
      ...
      jQuery(document).on('click', 'input.select-img', function( event ){
        ...
        var eCommerceGemImage = wp.media.controller.Library.extend({
            defaults :  _.defaults({
        ...
        ecommerce_gem_file_frame = wp.media.frames.ecommerce_gem_file_frame = wp.media({
          button: {
            text: jQuery( this ).data( 'uploader_button_text' )
          },
          state : 'rtam-insert-image',
              states : [
                  new eCommerceGemImage()
              ],
          multiple: false  // Set to true to allow multiple files to be selected
        });

        // When an image is selected, run a callback.
        ecommerce_gem_file_frame.on( 'select', function() {

          var state = ecommerce_gem_file_frame.state('rtam-insert-image');
